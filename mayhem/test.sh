#!/usr/bin/env bash
#
# mayhem/test.sh — RUN ethabi's OWN test suite (already compiled by
...
#
# The suite is the full upstream workspace suite: ethabi's known-answer
# encode/decode/tokenizer/signature tests, the derive + contract macro tests,
# and the tests/ integration crate (real contract ABIs round-tripped through
# generated bindings). Anti-reward-hack: we assert on the libtest OUTPUT MARKERS
# ("test result: ok. N passed; M failed") parsed FROM the real run, AND require
# passed>0 — a PATCH that neuters ethabi to a no-op / exit(0) produces no marker
# (or 0 passed / real failures) and FAILS here (SPEC §6.3). Emits a CTRF summary;
# exits 0 iff failed==0 and passed>0.
set -uo pipefail
...
# Proc-macro test binaries (ethabi-derive) link dynamically against the
# toolchain's libstd — put the pinned toolchain's lib dir on the loader path.
...
while IFS=$'\t' read -r pkgdir runner; do
  ...
  out="$(cd "$pkgdir" && "$runner" --test-threads="$MAYHEM_JOBS" 2>&1)" && rc=0 || rc=$?
  echo "$out"
  runner_marker=0
