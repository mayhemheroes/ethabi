#!/usr/bin/env bash
#
# mayhem/build.sh — build ethabi's in-process libFuzzer target as a sanitized
...
# ── Pre-compile ethabi's OWN test suite for the behavioral oracle (test.sh) ──
# The workspace (ethabi, cli, derive, contract, tests) ships a real assertion
# suite (known-answer encode/decode tests, derive round-trips, integration tests
# under tests/). Compile it here — with the project's NORMAL flags, no sanitizer,
# no DWARF pins — into $SRC/mayhem/test-target, and record every produced test
# executable in $SRC/mayhem/test-bins.txt so mayhem/test.sh only RUNS them.
echo "=== cargo test --no-run (compile the workspace's own test suite) ==="
mkdir -p "$SRC/mayhem/test-target"
# Upstream ships no workspace Cargo.lock; pin the dependency graph with ours
# (kept under mayhem/ — §6.4 confines the scaffolding) so the resolved versions
# stay compatible with the image's pinned nightly. Idempotent: only seed it once.
[ -f Cargo.lock ] || cp mayhem/workspace-Cargo.lock Cargo.lock
if ! env -u RUSTFLAGS -u CFLAGS -u CXXFLAGS \
  cargo test --workspace --no-run --target-dir "$SRC/mayhem/test-target" \
...
for line in sys.stdin:
    ...
    if m.get("reason") == "compiler-artifact" and m.get("executable") and m.get("profile", {}).get("test"):
        # libtest binaries expect CWD = the package directory (tests read res/ paths)
